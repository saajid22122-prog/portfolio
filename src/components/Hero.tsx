  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-8 md:px-16 lg:px-24 flex justify-between items-center">
        ...
        <div className="font-display text-xl tracking-[0.15em] text-[#F5F1EA]/80 font-light">
          S<span className="text-cyan-400/60">.</span>K
        </div>

        {/* Desktop Navigation - elegant and sparse */}
        ...
        {/* Mobile menu button */}
        <button className="md:hidden text-white/60 hover:text-white/90" onClick={() => setIsOpen(!isOpen)}>
        ...
        >
          {sections.map((s) => (
            <button
              key={s}
              onClick={() => scrollTo(s)}
              className="text-left text-sm tracking-wide text-white/60 hover:text-white/90 py-2"
            >
              {s}
            </button>
          ))}
